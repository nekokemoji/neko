...
当前 Neko 版本为 **1.8.0**。`versions.env` 冻结：
...
按选择顺序执行，避免把 VPS 瞬间打满。每组 ICMP 最长 30 秒，所有子进程都有外层
时限。双栈选择全部六地时共 36 条路径和 36 组丢包样本；若目标连续超时，整组测试
可能接近 7 分钟。
...
未完整发出 100 包、`ping` 缺失或输出无法可靠解析时显示“未完成/未测”，不会拿部分
样本冒充完整结果。目标或元数据服务拒绝探测时也只降级，不根据缺失数据硬猜。
