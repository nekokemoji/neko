...
| 不知道 VPS 线路好不好 | 从当前真实 IPv4/IPv6 源地址测试六地区三网回程，查看每线 100 包的 P50/P95、波动、丢包和双栈对比 |
...
- 每个目标先进行 3 包 ICMP 预检；有响应后才发送固定 100 包，并显示最小值、平均值、P50、P95、最大值和波动；
- 预检没有 ICMP 响应时不会直接冒充“100% 实际丢包”；如果 TCP 路径仍有响应，会明确显示为“未确认”；
- 双栈且同一运营商两族样本都完整时，会优先比较丢包，再用保守阈值比较 P95，并给出本地应先测试哪条入站订阅的参考；
- ASN 可以辅助识别 163、CN2、169、9929、CMI、CMNET、CMIN2，但单个 ASN 不能证明具体优化产品；
- “原生 IP”“住宅 IP”“解锁”没有跨数据库通用的权威定义，Neko 会显示证据和分歧，不会硬猜。

这里比较的 IPv4/IPv6 仍然是 **VPS 回程参考**，不能代替你自己设备到 VPS 的入站
实测。线路检测会按当前 Neko 安装状态工作：VPS 没有配置公网 IPv4 时自动跳过 IPv4，
继续测试 IPv6，不会因为缺少一族而崩掉。
...
| 7 | VPS 硬件、IP 与网络体检 | 查看服务、IP、BGP、六地区三网回程、100 包统计与双栈参考 |
