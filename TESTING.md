...
- Bootstrap 离线解压固定源码包、核对 1.13.1 标记与 AKDNS 运行组件并清理临时目录；模拟精简系统缺少 `tar`/`gzip` 时会先通过系统包管理器补齐。
- AKDNS 只使用固定的 3.0.0 上游提交与 SHA-256；隔离系统根目录动态覆盖永久启用、精确恢复、无改动、未知 DNS、上游非零退出、递归健康失败和 Neko 后置验证失败，确认公网严格 DNS 改用中立解析器，IPv4 出口订阅改用 AKDNS，升级时能从 root-only 状态自动重渲染，并确认 `/etc/resolv.conf` 符号链接、订阅、管理状态、`nsswitch.conf`、NetworkManager 文件及 resolver 服务状态完整恢复。
...
- 三个服务端核心都保持 IPv4 出口使用系统解析器，并把 IPv6 出口隔离到 `2606:4700:4700::1111` 的校验证书 DoH；结构化断言覆盖 IPv4-only、IPv6-only、dual，确认 IPv4 不绕过 AKDNS、IPv6 不再继承系统 AKDNS。
- sing-box 和 Hysteria 在 TCP 80/443、UDP 443 开启有限域名恢复；Xray 的全部入站明确启用 HTTP/TLS/QUIC 目标覆盖。冻结核心会解析这些配置，测试同时确认恢复后仍由对应出口的严格 DNS 与异族拒绝处理。
- sing-box 服务端 IPv4 本地 DNS 明确使用 Go 解析路径，避免 Debian 12 镜像中的 systemd-resolved D-Bus 异常拖死 IPv4 域名出口；IPv6 域名出口不经过该系统路径。
- Hysteria 四个方向分别使用所需的 `mode: 4`/`mode: 6` 和 `bindIPv4`/`bindIPv6`；只有 `mode: 6` 配置包含独立 IPv6 DoH。用假核心动态验证监管脚本可启动单个子进程，也可同时启动四个进程并在任一退出时终止其余进程，交由 systemd 重启完整进程组。
...
- 面板第 7 项列出 GOECS、NodeQuality 与 Neko 三网线路检测；前两项只下载固定提交的入口并校验 SHA-256，显示仍存在的可变传递下载，默认取消且必须输入各自专用确认词才以 root 运行。离线替身覆盖成功、取消和哈希不匹配拒绝；Neko 线路菜单严格使用广东、上海、北京、四川、湖北、辽宁、全部地区的顺序。
...
- AnyReality 在新安装和旧版升级时默认启用，使用独立端口、密码、REALITY 密钥和严格出口路由；仅加入 sing-box 与 Shadowrocket，Mihomo、Stash 保持不变，防火墙同步开放对应 TCP 端口。面板不提供 AnyReality 卸载入口；功能 9 现为独立的可选 AKDNS 事务入口。
- 控制面板端点刷新在地址未变化时不重启；模拟新地址更新成功、服务失败后完整回滚，以及回滚服务仍失败时保留状态备份。
- 卸载流程永不删除共享维护锁文件；真实并发测试证明第一个维护进程完全退出前，第二个进程无法通过同一路径或新 inode 进入，并在完整 VM 中复验。
- 控制面板从 IPv4-only 补装 IPv6 的成功、第二组跨族端口无碰撞、两个跨族令牌、证书扩容、重复请求无操作、服务失败后配置/证书/firewalld 自动回滚；恢复路径还会拒绝根目录等危险目标。
- 分别从 schema 1、schema 2 与 schema 3 的双栈/IPv4-only/IPv6-only 模拟升级到 Neko 1.13.1，确认原端口、协议凭据、REALITY 参数、安装模式、令牌和旧订阅 URL 继续可用；升级安装 AKDNS 事务组件，只新增 AnyReality 及旧版缺少的跨族/Trojan 数据。schema 4 已有 AnyReality 且端口正在监听时，升级仍原样保留其身份，同时继续拒绝状态文件中的真实端口重叠。
- 升级开始任何暂存、备份或服务操作前，会核对目标与已安装五个核心版本/发布资产 SHA 清单以及实际二进制报告的版本身份；目标需要更换核心或机器版本身份偏离清单时明确拒绝，并证明状态、配置、脚本、证书和服务均未变化。
...
systemd，再运行平台识别、全部 Shell 语法、AKDNS 隔离系统根的启用/恢复/失败回滚、
schema 4 四线路渲染、250 次 ACME 快速失败、限额/超时保护、systemd unit 解析和实际
临时 unit 启动。Debian 12 还会安装
...
- 8 个发行版真实接管到公共 AKDNS、UDP/TCP 53 可达性、流媒体区域结果与长期稳定性；
