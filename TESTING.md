...
- Bootstrap 离线解压固定源码包、核对 1.2.4 标记并清理临时目录；模拟精简系统缺少 `tar`/`gzip` 时会先通过系统包管理器补齐。
...
- 真实 `sing-box check` 同时解析服务端配置和两份客户端 Remote Profile，另有 `xray run -test`、`caddy validate`。
- Hysteria 的 IPv4/IPv6 两份配置分别读取并执行到端口跳跃帮助程序查找阶段；测试刻意不给它 nftables/iptables，避免改动宿主防火墙。
- 真实 Mihomo 分别解析严格 IPv4 与严格 IPv6 配置。
- 订阅目录恰好生成 8 个文件：Mihomo、Stash、Shadowrocket、sing-box 各 v4/v6 两份。
...
- 分别从 schema 1 / Neko 1.0.x、schema 2 / Neko 1.1.x 和实际上一版 Neko 1.2.3 模拟升级到 Neko 1.2.4，确认端口、协议凭据、REALITY 参数和订阅令牌不变，旧订阅文件被替换为 8 个新文件，并安装 Hysteria 双进程监管脚本与单元。
...
2. 8 个发行版镜像分别在 amd64 与 QEMU arm64 用户空间运行全部 Shell 语法解析、真实 `/etc/os-release` 平台检测，并实际渲染和结构化检查 8 份订阅，共 16 个组合；amd64 另外从缺少工具的镜像执行一次离线 Bootstrap，真实调用该发行版的包管理器。

矩阵目标：

| 发行版镜像 | amd64 | arm64/QEMU |
|---|---:|---:|
| Debian 12 | 平台 + 订阅渲染 | 平台 + 订阅渲染 |
| Debian 13 | 平台 + 订阅渲染 | 平台 + 订阅渲染 |
| Ubuntu 24.04 | 平台 + 订阅渲染 | 平台 + 订阅渲染 |
| Ubuntu 26.04 | 平台 + 订阅渲染 | 平台 + 订阅渲染 |
| Rocky Linux 9 | 平台 + 订阅渲染 | 平台 + 订阅渲染 |
| Rocky Linux 10 | 平台 + 订阅渲染 | 平台 + 订阅渲染 |
| AlmaLinux 9 | 平台 + 订阅渲染 | 平台 + 订阅渲染 |
| AlmaLinux 10 | 平台 + 订阅渲染 | 平台 + 订阅渲染 |
...
容器和 QEMU user-mode 很适合发现 Bash、架构、系统识别和配置格式问题，但不能代替 systemd、内核网络、防火墙、ACME 与移动客户端真机测试。首次部署应使用可随时重装的测试 VPS，导入八条订阅逐项验收后再长期使用。
