...
- Bootstrap 离线解压固定源码包、核对 1.12.1 标记与 AKDNS 运行组件并清理临时目录；模拟精简系统缺少 `tar`/`gzip` 时会先通过系统包管理器补齐。
- AKDNS 只使用固定的 3.0.0 上游提交与 SHA-256；隔离系统根目录动态覆盖永久启用、精确恢复、无改动、未知 DNS、上游非零退出和 Neko 后置验证失败，确认生产配置/订阅目录已初始化，并确认 `/etc/resolv.conf` 符号链接、`nsswitch.conf`、NetworkManager 文件及 resolver 服务状态完整恢复。
...
- 分别从 schema 1、schema 2 与 schema 3 的双栈/IPv4-only/IPv6-only 模拟升级到 Neko 1.12.1，确认原端口、协议凭据、REALITY 参数、安装模式、令牌和旧订阅 URL 继续可用；升级安装 AKDNS 事务组件，只新增 AnyReality 及旧版缺少的跨族/Trojan 数据。schema 4 已有 AnyReality 且端口正在监听时，升级仍原样保留其身份，同时继续拒绝状态文件中的真实端口重叠。
