...
NEKO_SOURCE_COMMIT="eac9e0b7129762c3957a34ee488bcdf6ec26d1f1"
...
if [[ -n "$NEKO_BOOTSTRAP_ARCHIVE" ]]; then
  ...
  printf '[信息] 正在从 GitHub 下载固定版本 Neko 1.7.3……\n'
...
for required_file in \
  ...
  runtime/panel.sh runtime/diagnostics.sh runtime/renew.sh runtime/hysteria-dual.sh \
...
grep -Fq 'NEKO_RELEASE="1.7.3"' "$WORKDIR/source/versions.env" \
  || die_bootstrap "下载的项目版本不是预期的 Neko 1.7.3。"
